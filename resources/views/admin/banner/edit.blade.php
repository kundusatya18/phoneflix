                    ...
                    <div class="tile-body">
                        <div class="form-group">
                            ...
                            <select class="form-control @error('category_id') is-invalid @enderror" name="category_id" id="category_id">
                                ...
                                <option value="{{$n->id}}"@if($n->id==$targetBanner->category_id){{"selected"}}@endif>{{$n->name}}</option>
                                @endforeach
                            </select>
                            @error('category_id') {{ $message ?? '' }} @enderror
                        </div>
                        <div class="form-group">
                            <label class="control-label" for="name">Select Genre<span class="m-l-5 text-danger"> *</span></label>
                            <select class="form-control @error('genre_id') is-invalid @enderror" name="genre_id" id="genre_id">
                                <option value=""> Select Category </option>
                                @foreach($genre as $n)
                                <option value="{{$n->id}}"@if($n->id==$targetBanner->genre_id){{"selected"}}@endif>{{$n->name}}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            <div class="row">
                                ...
                                <div class="col-md-10">
                                    ...
                                    <input class="form-control @error('image') is-invalid @enderror" type="file" id="image" name="image" value="{{ old('image', $targetBanner->image) }}"/>
